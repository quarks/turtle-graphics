...
const [newFormat, addText] = (function tt() {
    const { PI, atan, min, sqrt } = Math;
    const r2d = 180 / PI;
    const m = new Map();

    function format(weight = 8, height = 120, scale = 1,
        leading = 0, kerning = 0) {
        return {
            weight: weight, height: height, scale: scale, leading: leading,
    ...
    /* Add text at the current turtle position. */
    function addText(t, text, fmt = format()) {
        let { weight, height, scale, leading, kerning, align, line_width } = fmt;
        ...
        lines.forEach(line => {
            ...
            for (let i = 0; i < line.length; i++) {
                ...
                if (ch) {
                    ...
                    ch.f(t, weight * scale, height / 10, ch.w * scale);
                    t.dxy(kerning, 0);
    ...
    function addCharacterBox(t, cw, fmt) {
        let { weight, height, scale } = fmt;
        let ah = 0.7 * height, ad = 0.3 * height, pw = weight * cw * scale;
        t.push_pen().animateoff().pd().f(0).pensize(1).pencolor('red')
            .fd(pw).lt(90).fd(ah).lt(90).fd(pw).lt(90).fd(height).lt(90).fd(pw)
            .lt(90).fd(ad).pop_pen();
        return t;
    }
    ...
    function _32(t, s, h, width) {
        initChar(t, s * width).pop_pen();
    }

    m.set('!', { w: 3, f: _33 });
    function _33(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -6 * h).f(90).cap(SQUARE).pd()
            .fd(4 * h).pu().cap(BUTT).fd(1.5 * h).pd().fd(h).pop_pen();
    }

    m.set('"', { w: 3.5, f: _34 });
    function _34(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6.25 * h).f(90).cap(SQUARE).pd()
            .fd(1.5 * h).dxy(1.5 * s, -1.5 * h).pd().fd(1.5 * h).pop_pen();
    }

    m.set('#', { w: 6.5, f: _35 });
    function _35(t, s, h, width) {
        initChar(t, s * width).dxy(3 * s, -6 * h).cap(SQUARE)
    ...
    m.set('$', { w: 5, f: _36 });
    function _36(t, s, h, width) {
        let rx = 1.5 * s, ry = 1.25 * h;
        initChar(t, s * width).dxy(2.5 * s, -3 * h).push_pen()
    ...
    m.set('%', { w: 6, f: _37 });
    function _37(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -6 * h).pd().cap(SQUARE)
    ...
    m.set('&', { w: 5.5, f: _38 });
    function _38(t, s, h, width) {
        initChar(t, s * width).dxy(4.5 * s, -3.5 * h).f(90).pd().fd(1.5 * h)
    ...
    m.set('\'', { w: 2, f: _39 });
    function _39(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6.25 * h).f(90).cap(SQUARE).pd()
            .fd(1.5 * h).pop_pen();
    }

    m.set('(', { w: 3, f: _40 });
    function _40(t, s, h, width) {
        initChar(t, s * width)
    ...
    m.set(')', { w: 3, f: _41 });
    function _41(t, s, h, width) {
        initChar(t, s * width).dxy(0.5 * s, -6.5 * h).f(0).pd()
            .oval(1.5 * s, 4 * h, 180, RT).pop_pen();
    }

    m.set('*', { w: 5, f: _42 });
    function _42(t, s, h, width) {
    ...
    m.set(',', { w: 2, f: _44 });
    function _44(t, s, h, width) {
        initChar(t, s * width).dxy(s, -s).f(90).pd().fd(s)
            .cap(ROUND).pensize(0.65 * s).goby(-0.75 * s, 1.25 * h).pop_pen();
    }

    m.set('-', { w: 3.5, f: _45 });
    function _45(t, s, h, width) {
        initChar(t, s * width).dxy(0.5 * s, - 3.5 * h).pd().fd(2.5 * s)
            .pop_pen();
    }

    m.set('.', { w: 2, f: _46 });
    function _46(t, s, h, width) {
        initChar(t, s * width).dxy(s, -s).f(90).pd().fd(s)
            .pop_pen();
    }

    m.set('/', { w: 3.5, f: _47 });
    function _47(t, s, h, width) {
        initChar(t, s * width).dxy(2.8 * s, -6 * h).cap(SQUARE).pd()
            .goby(-1.5 * s, 7.5 * h).pop_pen();
    }

    m.set('0', { w: 5, f: _48 });
    function _48(t, s, h, width) {
        initChar(t, s * width).dxy(2.5 * s, -6 * h).pd().oval(1.5 * s, 3 * h)
            .pop_pen();
    }

    m.set('1', { w: 5, f: _49 });
    function _49(t, s, h, width) {
        initChar(t, s * width).dxy(s, -5.3 * h)
            .pd().oval(1.5 * s, h, 90, LT).f(90).fd(6.3 * h)
            .dxy(-1.5 * s, 0).f(0).fd(3 * s).pop_pen();
    }

    m.set('2', { w: 5, f: _50 });
    function _50(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4 * h).f(270).pd()
    ...
    m.set('3', { w: 5, f: _51 });
    function _51(t, s, h, width) {
        initChar(t, s * width).dxy(s, -5 * h).f(270).pd()
    ...
    m.set('4', { w: 5, f: _52 });
    function _52(t, s, h, width) {
        initChar(t, s * width).dxy(4.5 * s, -2 * h).f(180).pd()
            .fd(1.5 * s).cap(ROUND).fd(2 * s).goby(2.5 * s, -4 * h)
            .cap(BUTT).goby(0, 6 * h).fd(0.5 * s).pop_pen();
    }

    m.set('5', { w: 5, f: _53 });
    function _53(t, s, h, width) {
        initChar(t, s * width).dxy(3.5 * s, -6 * h).f(180).pd().cap(SQUARE)
            .fd(2 * s).lt(90).fd(2.5 * h).lt(90).fd(s)
            .oval(1.5 * s, 1.75 * h, 220, RT).pop_pen();
    }

    m.set('6', { w: 5, f: _54 });
    function _54(t, s, h, width) {
        initChar(t, s * width).dxy(s, -1.75 * h).f(270).pd()
            .oval(1.75 * h, 1.5 * s, 360, RT).oval(4.25 * h, 1.2 * s, 90, RT)
            .oval(2 * s, 1.5 * h, 60, RT).pop_pen();
    }

    m.set('7', { w: 5, f: _55 });
    function _55(t, s, h, width) {
        initChar(t, s * width).dxy(0.5 * s, -6 * h).pd()
            .fd(4 * s).dxy(-0.5 * s, 0.4 * h).cap(ROUND)
            .goby(-1.7 * s, 3 * h).cap(BUTT).goby(-1.7 * s, 3 * h)
            .pop_pen();
    }

    m.set('8', { w: 5, f: _56 });
    function _56(t, s, h, width) {
        initChar(t, s * width).dxy(2.5 * s, -3 * h).f(180).pd()
            .oval(1.5 * s, 1.5 * h, 360, RT).oval(1.75 * s, 1.5 * h, 360, LT)
            .pop_pen();
    }

    m.set('9', { w: 5, f: _57 });
    function _57(t, s, h, width) {
        initChar(t, s * width).dxy(4 * s, -4.25 * h).f(90).pd()
    ...
    m.set(':', { w: 2, f: _58 });
    function _58(t, s, h, width) {
        initChar(t, s * width).dxy(1 * s, -4 * h).f(90)
            .pd().fd(s).dxy(0, -2 * s + 4 * h).fd(s).pop_pen();
    }

    m.set(';', { w: 2, f: _59 });
    function _59(t, s, h, width) {
        initChar(t, s * width).dxy(1 * s, -4 * h).f(90)
            .pd().fd(s).dxy(0, -2 * s + 4 * h).fd(s)
            .cap(ROUND).pensize(0.65 * s).goby(-0.75 * s, 1.25 * h)
            .pop_pen();
    }

    m.set('<', { w: 6, f: _60 });
    function _60(t, s, h, width) {
        let dx = 4.5 * s, dy = 2 * h, hyp = sqrt(dx * dx + dy + dy);
        let a = atan(dy / dx); if (t._degrees) a = r2d * a;
        initChar(t, s * width).dxy(5 * s, -4.5 * h).f(180).pd().lt(a).fd(hyp / 2)
            .cap(ROUND).fd(hyp / 2).lt(180 - 2 * a).fd(hyp / 2).cap(BUTT)
            .fd(hyp / 2).pop_pen();
    }

    m.set('=', { w: 6, f: _61 });
    function _61(t, s, h, width) {
        let dx = 4 * s;
        initChar(t, s * width).dxy(s, -3.5 * h).f(0).pd().fd(dx)
            .dxy(-dx, 1.75 * h).fd(dx).pop_pen();
    }

    m.set('>', { w: 6, f: _62 });
    function _62(t, s, h, width) {
        let dx = 4.5 * s, dy = 2 * h, hyp = sqrt(dx * dx + dy + dy);
        let a = atan(dy / dx); if (t._degrees) a = r2d * a;
        initChar(t, s * width)
            .dxy(s, -4.5 * h).f(0).pd()
    ...
    m.set('?', { w: 5, f: _63 });
    function _63(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4.5 * h).f(270).pd()
            .oval(1.5 * h, 1.5 * s, 270, RT).cap(SQUARE).lt(90).fd(1.5 * h)
            .cap(BUTT).dxy(0, h).fd(h).pop_pen();
    }

    m.set('@', { w: 8, f: _64 });
    function _64(t, s, h, width) {
        initChar(t, s * width)
            .dxy(5.1 * s, -0.45 * h).f(90).pd().fd(0.2 * h)
            .oval(0.15 * h, 1.255 * s, 360, RT).fd(0.1 * h)
            .oval(0.05 * h, s, 180, LT).fd(0.1 * h)
            .oval(0.35 * h, 3.2 * s, 180, LT).oval(0.3 * h, 3.25 * s, 110, LT)
            .pop_pen();
    }

    m.set('A', { w: 5.9, f: _65 });
    function _65(t, s, h, width) {
        ...
        initChar(t, s * width).dxy(s2, 0).f(0).cap(SQUARE).pd().lt(a).fd(l13)
            .cap(ROUND).fd(l23).rt(180 - 2 * b).fd(l23).cap(ROUND).fd(l13)
            .pu().bk(l13).rt(90 + b).pd().fd(4 * dx / 3).pop_pen();
    }

    m.set('B', { w: 6, f: _66 });
    function _66(t, s, h, width) {
        initChar(t, s * width).dxy(s, 0).f(270).cap(SQUARE)
    ...
    m.set('C', { w: 6.2, f: _67 });
    function _67(t, s, h, width) {
        initChar(t, s * width).dxy(s, -3 * h).pd()
            .push_pen().f(270).oval(3 * h, 2.4 * s, 139, RT).pop_pen().f(90)
            .oval(3 * h, 2.4 * s, 135, LT).pop_pen()
    }

    m.set('D', { w: 6.5, f: _68 });
    function _68(t, s, h, width) {
        initChar(t, s * width).dxy(s, 0).f(270).cap(SQUARE)
    ...
    m.set('E', { w: 5, f: _69 });
    function _69(t, s, h, width) {
        initChar(t, s * width).dxy(4 * s, -6 * h).f(180).pd().cap(SQUARE)
            .fd(3 * s).lt(90).fd(6 * h).lt(90).fd(3 * s)
            .dxy(-3 * s, -3.25 * h).f(0).fd(2.5 * s).pop_pen();
    }

    m.set('F', { w: 5, f: _70 });
    function _70(t, s, h, width) {
        initChar(t, s * width).dxy(4 * s, -6 * h).f(180).pd().cap(SQUARE)
    ...
    m.set('G', { w: 6, f: _71 });
    function _71(t, s, h, width) {
        initChar(t, s * width).dxy(3.5 * s, -6 * h).pd()
    ...
    m.set('H', { w: 6, f: _72 });
    function _72(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6 * h).f(90).pd().cap(SQUARE)
            .fd(6 * h).dxy(4 * s, -6 * h).fd(6 * h).dxy(-4 * s, -3.3 * h)
            .f(0).cap(BUTT).fd(4 * s).pop_pen();
    }

    m.set('I', { w: 3, f: _73 });
    function _73(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -6 * h).f(90).pd().cap(SQUARE)
    ...
    m.set('K', { w: 5.5, f: _75 });
    function _75(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6 * h).f(90).pd().cap(SQUARE)
            .fd(6 * h).dxy(0, -2.2 * h).cap(BUTT).goby(3.5 * s, -3.9 * h)
            .dxy(-2.5 * s, 3.3 * h).goby(3 * s, 3 * h)
            .pop_pen();
    }

    m.set('L', { w: 5, f: _76 });
    function _76(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6 * h).f(90).pd().cap(SQUARE)
            .fd(6 * h).lt(90).fd(3 * s)
            .pop_pen();
    }
    ...
    m.set('M', { w: 7, f: _77 });
    function _77(t, s, h, width) {
        initChar(t, s * width).dxy(s, 0).f(270).pd().cap(SQUARE)
    ...
    m.set('U', { w: 6, f: _85 });
    function _85(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6 * h).pd().f(90).cap(SQUARE)
            .fd(4 * h).dxy(0, 0.5 * h).oval(1.5 * h, 2 * s, 180, LT)
            .cap(SQUARE).fd(4.5 * h)
            .pop_pen();
    }

    m.set('V', { w: 6, f: _86 });
    function _86(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6 * h).pd().cap(SQUARE)
    ...
    m.set('[', { w: 3.3, f: _91 });
    function _91(t, s, h, width) {
        initChar(t, s * width).dxy(2.5 * s, -6 * h).f(180).cap(SQUARE).pd()
            .fd(1.5 * s).lt(90).fd(7.5 * h).lt(90).fd(1.5 * s).pop_pen();
    }

    m.set('\\', { w: 3.5, f: _92 });
    function _92(t, s, h, width) {
        initChar(t, s * width).dxy(0.7 * s, -6 * h).cap(SQUARE).pd()
            .goby(1.5 * s, 7.5 * h).pop_pen();
    }

    m.set(']', { w: 3.3, f: _93 });
    function _93(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6 * h).f(0).cap(SQUARE).pd()
            .fd(1.5 * s).rt(90).fd(7.5 * h).rt(90).fd(1.5 * s).pop_pen();
    }

    m.set('^', { w: 5.5, f: _94 });
    function _94(t, s, h, width) {
        let dh = 1.5 * h;
        initChar(t, s * width).dxy(0.5 * s, -3 * h).pd()
            .goby(s, -dh).cap(ROUND).goby(s, -dh).goby(s, dh).cap(BUTT)
            .goby(s, dh).pop_pen();
    }

    m.set('_', { w: 5, f: _95 });
    function _95(t, s, h, width) {
        initChar(t, s * width).dxy(0, 0.12 * h).pd().goby(5 * s, 0).pop_pen();
    }

    m.set('`', { w: 2, f: _96 });
    function _96(t, s, h, width) {
        initChar(t, s * width).dxy(0.75 * s, -6.25 * h).f(90).cap(SQUARE).pd()
            .goby(0.25 * s, 1.5 * h).pop_pen();
    }

    m.set('a', { w: 5, f: _97 });
    function _97(t, s, h, width) {
        initChar(t, s * width).dxy(s, -3.5 * h).f(270).pd().cap(BUTT)
            .oval(0.5 * h, 1.5 * s, 180, RT).fd(h).cap(SQUARE)
            .fd(2.5 * h).cap(BUTT).dxy(0, -2.5 * h).f(180).fd(s)
            .oval(2 * s, 1.25 * h, 180, LT).oval(s, h, 90, LT).pop_pen();
    }

    m.set('b', { w: 5.2, f: _98 });
    function _98(t, s, h, width) {
        initChar(t, s * width).dxy(s, 0).push_pen()
    ...
    m.set('c', { w: 5.2, f: _99 });
    function _99(t, s, h, width) {
        initChar(t, s * width).dxy(4.5 * s, -3.0 * h).f(270).pd().cap(BUTT)
    ...
    function _100(t, s, h, width) {
        initChar(t, s * width).dxy(4.5 * s, 0).push_pen()
            .dxy(0, -3.5 * h).f(270).pd().cap(BUTT)
    ...
    function _101(t, s, h, width) {
        initChar(t, s * width).dxy(s, -2 * h).pd().cap(SQUARE)
            .fd(3 * s).f(270).cap(BUTT).oval(2 * h, 1.5 * s, 270, LT)
            .oval(1.5 * s, -h, 90, LT).pop_pen();
    }

    m.set('f', { w: 4, f: _102 });
    function _102(t, s, h, width) {
        initChar(t, s * width).dxy(3.5 * s, -6 * h).pd().f(180)
    ...
    function _103(t, s, h, width) {
        initChar(t, s * width).dxy(4.5 * s, 0).push_pen().dxy(0, -3.5 * h)
    ...
    m.set('i', { w: 3, f: _105 });
    function _105(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -4 * h).pd().f(90).cap(SQUARE)
            .fd(4 * h).cap(BUTT).dxy(0, -6 * h).fd(h)
            .pop_pen();
    }

    m.set('j', { w: 3, f: _106 });
    function _106(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -4 * h).pd().f(90).cap(SQUARE)
    ...
    m.set('l', { w: 3, f: _108 });
    function _108(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -6 * h).pd().f(90).cap(SQUARE)
            .fd(6 * h).pop_pen();
    }

    m.set('m', { w: 8, f: _109 });
    function _109(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4 * h).f(90).pd().cap(SQUARE).fd(4 * h)
        for (let i = 0; i < 2; i++)
    ...
    m.set('o', { w: 5, f: _111 });
    function _111(t, s, h, width) {
        initChar(t, s * width).dxy(2.5 * s, -4 * h).pd()
            .oval(1.65 * s, 2 * h, 360, RT).pop_pen();
    }

    m.set('p', { w: 5.2, f: _112 });
    function _112(t, s, h, width) {
        initChar(t, s * width).dxy(s, 0).push_pen().dxy(0, -3.5 * h).f(270)
    ...
    m.set('r', { w: 4, f: _114 });
    function _114(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4 * h).f(90).pd().cap(SQUARE).fd(4 * h)
            .dxy(0, -3 * h).f(270).oval(h, 2 * s, 90, RT).pop_pen();
    }

    m.set('s', { w: 5, f: _115 });
    function _115(t, s, h, width) {
        let rx = 1.5 * s;
        initChar(t, s * width).dxy(2.5 * s, -2 * h)
            .oval(rx, h, 100, LT).pd().oval(h, rx, 260, LT)
            .oval(rx, h, 260, RT).pop_pen();
    }

    m.set('t', { w: 5, f: _116 });
    function _116(t, s, h, width) {
        initChar(t, s * width).dxy(1.5 * s, -5.5 * h).pd().f(90).cap(SQUARE)
            .fd(2.5 * h).cap(BUTT).fd(2 * h).oval(h, 2 * s, 90, LT)
            .dxy(-3 * s, -4 * h).f(0).fd(3 * s).pop_pen();
    }

    m.set('u', { w: 5, f: _117 });
    function _117(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4 * h).pd().f(90).cap(SQUARE)
            .fd(h).cap(ROUND).fd(2 * h).oval(h, 1.5 * s, 180, LT)
            .dxy(0, -3 * h).f(90).cap(SQUARE).fd(4 * h).pop_pen();
    }

    m.set('v', { w: 5, f: _118 });
    function _118(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4 * h).pd().cap(SQUARE)
    ...
    function _121(t, s, h, width) {
        initChar(t, s * width).dxy(4 * s, -4 * h).pd().cap(SQUARE)
            .goby(-2 * s, 6 * h).dxy(-s, -6 * h).goby(0.888 * s, 2 * h)
            .cap(BUTT).goby(0.888 * s, 2 * h).pop_pen();
    }

    m.set('z', { w: 5, f: _122 });
    function _122(t, s, h, width) {
        initChar(t, s * width).dxy(s, -4 * h).cap(SQUARE).pd()
    ...
    m.set('{', { w: 4, f: _123 });
    function _123(t, s, h, width) {
        let dx = 1.5 * s, dy = 1.9 * h;
        initChar(t, s * width).dxy(3.5 * s, -6 * h).f(180).pd()
            .oval(-dx, dy, 90, LT).oval(dy, -dx, 90, RT).f(0)
            .oval(dx, dy, 90, RT).oval(dy, dx, 90, LT).pop_pen();
    }

    m.set('|', { w: 2, f: _124 });
    function _124(t, s, h, width) {
        initChar(t, s * width).dxy(s, -6.5 * h).cap(BUTT).pd().f(90)
            .fd(8.5 * h)
            .pop_pen();
    }

    m.set('}', { w: 4, f: _125 });
    function _125(t, s, h, width) {
        let dx = 1.5 * s, dy = 1.9 * h;
        initChar(t, s * width).dxy(0.5 * s, -6 * h).pd()
    ...
    m.set('~', { w: 4, f: _126 });
    function _126(t, s, h, width) {
        initChar(t, s * width).dxy(0.5 * s, -2.5 * h).f(270).pd()
            .oval(h, 0.75 * s, 180, RT).oval(h, 0.75 * s, 180, LT).pop_pen();
    }

    function initChar(t, w) {
        t.cap(BUTT).pu().dxy(w, 0).push_pen().dxy(-w, 0);
        return t;
    }
